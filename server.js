...
/* GREEN API: credenciales exclusivamente en Environment de Render. */
const GREEN_API_URL = String(process.env.GREEN_API_URL || "").trim().replace(/\/+$/, "");
const GREEN_ID_INSTANCE = String(process.env.GREEN_ID_INSTANCE || "").trim();
const GREEN_API_TOKEN = String(process.env.GREEN_API_TOKEN || "").trim();
const WHATSAPP_CHAT_ID = String(process.env.WHATSAPP_CHAT_ID || "").trim();
...
function whatsappConfigured(){return Boolean(WHATSAPP_ENABLED&&GREEN_API_URL&&GREEN_ID_INSTANCE&&GREEN_API_TOKEN&&WHATSAPP_CHAT_ID)}
function supabaseConfigured(){return Boolean(SUPABASE_URL&&SUPABASE_SERVICE_ROLE_KEY)}
function safeDestinationLabel(){if(!WHATSAPP_CHAT_ID)return"not configured";if(WHATSAPP_CHAT_ID.endsWith("@g.us"))return"group configured";return"configured"}
async function greenApiSend(message){
  if(!whatsappConfigured())return{ok:false,skipped:true,reason:"WHATSAPP_NOT_CONFIGURED"};
  const endpoint=`${GREEN_API_URL}/waInstance${GREEN_ID_INSTANCE}/sendMessage/${GREEN_API_TOKEN}`;
  const r=await fetch(endpoint,{method:"POST",headers:{"content-type":"application/json; charset=utf-8"},body:JSON.stringify({chatId:WHATSAPP_CHAT_ID,message,linkPreview:true})});
  const text=await r.text(); if(!r.ok)throw new Error(`GREEN API HTTP ${r.status}: ${text.slice(0,300)}`); whatsappLastError=""; return{ok:true};
}
function enqueueWhatsapp(message,kind){
  whatsappQueue=whatsappQueue.then(async()=>{const result=await greenApiSend(message);if(result.ok){if(kind==="alert"){whatsappSentAlerts++;whatsappLastAlertAt=Date.now()}else{whatsappSentDigests++;whatsappLastDigestAt=Date.now()}}return result}).catch(e=>{whatsappLastError=e.message;console.error("WhatsApp:",e.message);return{ok:false,error:e.message}});return whatsappQueue;
}
...
async function publishOperatorEvent(eventKey,eventType,title,message,metadata={},toWhatsapp=true){
  ...
  if(toWhatsapp)await enqueueWhatsapp(message,eventType==="channel_alert"?"alert":"digest");
  return true;
}
...
async function sendEveningOpening(forced=false){
  ...
  const msg=["📡 *CW LATAM · 40 M ESTÁ DESPERTANDO*","",`🌆 ${lead}`,"","🔮 *PREDICCIÓN TARDE / NOCHE*",`${ev.opening?"🟢":"🟡"} Condición esperada: *${bandLine(ev)}*`,`⭐ Mejor ventana estimada: *${win.text}*${win.historical?" · histórico CW LATAM":""}`,weatherLine(),"",`📊 Última hora: ${a.calls} estaciones · ${a.receivers} receptores${Number.isFinite(a.medianSnr)?` · SNR mediano ${a.medianSnr.toFixed(1)} dB`:""}`,`🌎 Rutas activas: ${topRoutes(a)}`,"","🧠 El aviso se dispara por tendencia de actividad, receptores y señal; no sólo por la hora.",localParts().dow===6?"📻 *Hoy 20:00 · LXCW NET CONTROL en 7.033 MHz*":"","","🎧 Buen momento para empezar a escuchar 40 m.",`🔗 ${CW_LATAM_URL}`].join("\n");
...
async function ensureNet(){
  const p=localParts(); if(p.dow!==6||p.hour<19||p.hour>21)return null;
...
async function netStart(){
  ...
  const msg=["🔴 *LXCW NET CONTROL · EN EL AIRE*","","📡 *7.033 MHz · CW QRS*","🕗 20:00–21:00","","🤖 NET CONTROL automático activo.","Llamá CQ normalmente en 7.033. El radar irá registrando los check-ins detectados por RBN.","","🔗 "+CW_LATAM_URL].join("\n");
...
async function recordNetSpot(spot){
  const p=localParts();if(p.dow!==6||p.hour!==20||!spot.isChannel)return;const n=await ensureNet();if(!n)return;if(n.status!=="live")await netStart();
...
async function netMid(){const n=await ensureNet();if(!n)return;const m=netMetrics(),ev=evaluatePropagation();const msg=["📻 *LXCW NET CONTROL · 20:30*","",`✅ *${m.participants} estaciones detectadas*`,`🌎 ${m.countries} países`,`👂 ${m.receivers} receptores`,"",`📈 40 m · *${bandLine(ev)}*`,"","Todavía estás a tiempo de sumarte en *7.033 MHz*.",`🔗 ${CW_LATAM_URL}`].join("\n");await publishOperatorEvent(`net:mid:${n.date}`,"net_mid","LXCW NET CONTROL",msg,{metrics:m,propagation:ev},true)}
async function netClose(){
  const n=await ensureNet();if(!n)return;const m=netMetrics(),reportsNow=await getPublicReportCount(),reports=(reportsNow!==null&&n.reportsStart!==null)?Math.max(0,reportsNow-n.reportsStart):0;n.status="closed";const msg=["🏁 *LXCW NET CONTROL · FINALIZADO*","","📡 7.033 MHz · 20:00–21:00",`✅ *${m.participants} estaciones detectadas*`,`🌎 *${m.countries} países*`,`👂 *${m.receivers} receptores*`,reports?`📊 *${reports} reportes comunitarios durante el NET*`:"",m.bestCall?`⚡ Mejor señal: *${m.bestCall} · ${m.bestSnr>=0?"+":""}${m.bestSnr} dB*`:"",m.widestCall?`🌎 Mayor cobertura: *${m.widestCall} · ${m.widestReceivers} receptores*`:"","","🧠 El resultado queda guardado para mejorar las futuras estimaciones de CW LATAM.","","Gracias a todos. *73* 📻",`🔗 ${CW_LATAM_URL}`].filter(Boolean).join("\n");
...
async function sundayNetSummary(){
  const p=localParts(),yesterday=localDateKey(-1);let s=lastNetSummary;
...
function nextOperatorAction(p=localParts()){
  ...
  for(const x of [{h:6,m:0,n:"Resumen nocturno + predicción"},{h:8,m:0,n:"Digest 2 h"},{h:10,m:0,n:"Digest 2 h"},{h:12,m:0,n:"Resumen mañana"},{h:14,m:0,n:"Digest 2 h"},{h:16,m:0,n:"Digest 2 h"},{h:18,m:45,n:"Predicción vespertina"},{h:20,m:0,n:d===6?"LXCW NET CONTROL":"Digest 2 h"},{h:22,m:0,n:"Digest 2 h"},{h:24,m:0,n:"Cierre diario"}]){let mins=(x.h===24?1440:x.h*60+x.m)-(h*60+m);if(mins<=0)mins+=1440;actions.push({...x,mins})}return actions.sort((a,b)=>a.mins-b.mins)[0];
}
...
async function operatorTick(){
  ...
  try{
    ...
    if(p.minute===0){if(p.hour===6)await send0600();else if([8,10,14,16,20,22].includes(p.hour)&&!(p.dow===6&&p.hour===20))await sendTwoHourDigest(p.hour);else if(p.hour===12)await send1200();else if(p.hour===0)await send0000()}
    // Proactividad 17:00–19:30. Primera tendencia convincente gana; fallback 18:45.
    if((p.hour===17||p.hour===18||(p.hour===19&&p.minute<=30))&&p.minute%5===0){const key=`evening_open:${p.date}`;if(!sentEventKeys.has(key)){const ev=evaluatePropagation();if(ev.opening)await sendEveningOpening(false);else if(p.hour===18&&p.minute>=45)await sendEveningOpening(true)}}
    if(p.dow===6){if(p.hour===19&&p.minute===30)await netPre(30);if(p.hour===19&&p.minute===55)await netPre(5);if(p.hour===20&&p.minute===0)await netStart();if(p.hour===20&&p.minute===30)await netMid();if(p.hour===21&&p.minute===0)await netClose()}
    if(p.dow===0&&p.hour===9&&p.minute===0)await sundayNetSummary();
